...
rts = function (context) {
    ...
    if (context.options[0]) {
		...
		// console.log(typeof config.prefix);
    }
    function plus() {
        level++;
    }
    ...
    function isValid(identifier) {
		return config.prefix.some(function(prefix) {
			return identifier.name.indexOf(prefix) >= 0;
		})
    }
    function testFunction(node) {
        ...
        for (i = 0, l = node.declarations.length; i < l; i++) {
            ...
            if (!isValid(identifier.id)) {
				...
				(config.prefix.length && config.prefix.length >= 2 ? "any of '" + config.prefix + "'" : '"' + config.prefix + '"') 
				 + '.');
            }
        }
    }
...
module.exports = function (context) {
    ...
    function testFunction(node) {
        ...
        for (i = 0, l = node.declarations.length; i < l; i++) {
            identifier = node.declarations[i];
            if (!isValid(identifier.id)) {
                context.report(identifier, 'Local variable "' + identifier.id.name + '" is not prefixed with "' + config.prefix + '".');
            }
        }
    }
    return {
